...
REX.UI.SVG.LedLabel = function (svgElem, args) {
    ...
    function refresh() {
        ...
        for (let item of values) {
            if ((Math.abs(item.value - value) < 1e-6)) {
                if (item.label.length > 0) {
                    svgLabel.textContent = item.label;
                }
                if(item.fill){
                    $svgLed.css('fill', item.fill);
                }
                if(item.stroke){
                    $svgLed.css('stroke', item.stroke);
                }                
                if(item.color){
                    $svgLabel.css('fill', item.color);
                }                                
                found = true;
                break;
            }
        ...
        if(!found){
            ...
            $svgLed.css('fill', that.COLORS.icon);
        }
    }
